...
状态：CLI 开发版已实现并完成最小真实链路验证；下文保留目标契约，当前已实现范围与剩余验收见第 12 节。
...
- npm 与 package-lock.json：直接依赖已固定版本，使用 `npm ci` 复现。
...
若该路径因上游协议变化无法通过，明确记录阻塞及证据，保留可工作的 DeepSeek + RAG 主路径；不得将 Antigravity 标为已支持。最小真实 OAuth refresh、工具回放和 Actor 混合生成已验证，具体证据与未验证项见第 12 节。
...
首期完成至少要求 A、B、C 和必要的分支恢复验证；D 只有通过才列入支持矩阵。项目设计第 49 节的故障与叙事场景仍为验收依据。当前已提供 CLI 实现；所有验收结论以运行证据为准，不能由接口存在推定通过。


## 12. 当前实现（2026-09-09）

实现入口为 [CLI](../src/cli/index.ts)。已实现并验证 DeepSeek 官方工具调用、SiliconFlow 完整 RAG、三层串行生成、场景草稿、知情子树过滤、逐 Status 审核、Git 结果发布与已接受结果恢复。详细证据见 [验收记录](validation/2026-09-09.md)。

本次冻结的关键接口补充：

- `reviewCandidate` 是 Orchestrator 提交前必需的只读审核工具，返回冻结正文与全部当前 Status；`accept` 必须对每个文档给出变更或明确的不变理由。
- `respondOOC` 处理纯系统交流，结果独立保存，不作为故事正文。
- `regenerate TURN --branch NAME` 从原 Input Commit 继承输入并建立独立运行记录；历史 Retcon 使用 `branch --from` 后编辑并提交。
- CLI 接受外部密钥文件；已经接受的结果恢复不需要密钥。运行预算落盘，显式延时和协议重试写入审计事件。
- Antigravity 为独立实现，使用已有加密登录的只读导入和刷新，保存原始返回片段。OAuth 客户端配置也从外部环境变量或私有文件提供，不内置到源码。已验证其担任 Actor，以及跨进程两次工具往返；其他 Agent 槽位尚未完成真实复杂 Schema 验收。
- Gemini 上游未表达的长度、数量、数值边界和 additionalProperties=false 由本地 Schema 严格验证；未知关键 Schema 语义明确拒绝。模型调用不允许省略必要的本地校验。

当前与目标设计的差距：完整执行库已持久化，但尚未自动生成 Memory；索引使用精确召回，仍在当前进程内计算，Worker 和规模优化未完成；CLI 暂不提供新 OAuth 浏览器登录和 Lore CRUD 命令，Lore 通过编辑文件、Git 提交及 index 管理。还需扩大真实故事、知情隔离、取消/故障、文学对照及规模验收。

## 13. 最小 Web 工作台（2026-09-09）

新增 Node HTTP 服务与无前端框架的响应式页面，不引入新的运行依赖。Web 服务仅管理指定父目录下的独立故事仓库；生成、检索、恢复和服务检查由子进程复用 CLI。任务使用随机 ID，在服务内缓存进度及结果，页面轮询；故事本身的持久记录仍由 CLI Journal 和 Git 管理。每个故事在 Web 侧禁止并发任务，文件操作还使用已有故事锁及基准版本检查。

设定编辑通过 Git 临时索引构造新版本后发布，拒绝脏工作区、版本变化与不在白名单内的路径。Lore / Status 删除进入 Git 历史，不提供运行日志或凭据的编辑接口。配置保存执行现有 Config、Status、Access 解析验证；Lore 检查必需字段。模型对叙事一致性的判断边界保持不变。

HTTP API 使用随机访问口令与 Authorization 请求头，默认仅监听回环地址，拒绝跨来源请求；密钥只从服务端配置读取。局域网监听必须在启动时显式指定。此版本面向个人使用，不包含多用户隔离、TLS 终止、OAuth 浏览器登录或独立后台任务调度服务。移动端采用单列布局、44px 操作目标和横向导航。
